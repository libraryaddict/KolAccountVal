...
export class Args {
  ...
  static fill<T extends ArgMap>(
    ...
    if (includeSettings) {
      metadata.traverseAndMaybeSet(args, (keySpec, key) => {
        ...
        const value_str = provider().retrieveCache(setting, "small_persist");
  ...
  static showHelp<T extends ArgMap>(
    ...
    provider().printHtml(`${metadata.scriptHelp}`);
    provider().printHtml("");
    provider().printHtml(
  ...
  static showGroupHelp<T extends ArgMap, S extends ArgMap>(
    ...
    provider().printHtml("");
    provider().printHtml(`<b>${group.name}:</b>`);
  }
  ...
  static showArgHelp<T extends ArgMap, S>(
    ...
    const nameText = `<font color='${provider().isDarkMode() ? "yellow" : "blue"}'>${
      arg.key ?? key
    }</font>`;
    const valueText =
